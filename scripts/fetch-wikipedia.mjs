...
const strip = (html) =>
  html
    ...
    // Краят на ред вътре в клетката е граница между имена, а не нищо. Без това
    // отборната клетка излизаше слепена: „RomaniaAndrei CorneaMarian Enache“
    // вместо „Romania, Andrei Cornea, Marian Enache“. Индивидуалната клетка не
    ...
    .replace(/\[\d+\]/g, '')
    .replace(/\s+/g, ' ')
    // Празните части между запетайките се махат, за да не остане „, ,“.
    .replace(/(?:\s*,\s*)+/g, ', ')
    // Пояснение в скоби на нов ред не е нов запис: „Not awarded, (as there
    // was a tie for gold)“ е едно изречение, не списък от две части.
    .replace(/,\s*\(/g, ' (')
    .replace(/^[\s,]+|[\s,]+$/g, '')
